...
check_paid <- soc_paid %>%
  bind_rows(ind_paid) %>%
  select(prop_ID, apellidos.x, apellidos.y, rptprop_razon_social.x, rptprop_razon_social.y, rptprop_nombre, Nombres.del.beneficiario, rptpro_ano, name_ranks, Acto.por.el.cual.se.otorgó, Fecha.de.Otorgamiento, namestringd, meets_string1, meets_string2, meets_string3, meets_string4)

paid_ids <- check_paid %>%
  select(prop_ID, Fecha.de.Otorgamiento)%>%
  separate(Fecha.de.Otorgamiento, into = c(NA, NA, "yr_paid"), sep = "/", remove = TRUE)%>%
  mutate(paid = 1)
library(rio)
export(paid_ids, "propertypaid_id.rds")

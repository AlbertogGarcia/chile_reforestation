...
submitby_contest <- ggplot(summarySE(follow_thru, measurevar="submitted_mp", groupvars=c("rptpro_tipo_concurso"), na.rm = TRUE), 
  ...
  geom_errorbar(aes(ymin=submitted_mp-se, ymax=submitted_mp+se), width=.25, position=pd) +
  xlab("contest")+
...
submitby_sex <- ggplot(summarySE(follow_thru, measurevar="submitted_mp", groupvars=c("female"), na.rm = TRUE), 
       aes(x=female, y=submitted_mp, fill=c("firebrick4", "green4", "blue"))
       ) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=submitted_mp-se, ymax=submitted_mp+se), width=.25, position=pd) +
  xlab("contest")+

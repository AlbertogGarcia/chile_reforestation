...
p_payments <- ggplot(summarySE(paid.enrolled, measurevar="paid", groupvars=c("first.treat","smallholder"), na.rm = TRUE), 
       aes(x=first.treat, y=paid, colour=smallholder)) + 
...
p_refpayments <- ggplot(summarySE(paid.enrolled, measurevar="paid", groupvars=c("first.treat","reftype"), na.rm = TRUE), 
       aes(x=first.treat, y=paid, colour=as.character(reftype))) + 
    geom_errorbar(aes(ymin=paid-se, ymax=paid+se), width=.1, position=pd) +
    geom_line(position=pd) +
    geom_point(size=3, shape=21, fill="white", position=pd)
...
p_forest <- ggplot(summarySE(enrolled_df, measurevar="forest", groupvars=c("first.treat","smallholder"), na.rm = TRUE), 
       aes(x=first.treat, y=forest, colour=smallholder)) + 
    geom_errorbar(aes(ymin=forest-se, ymax=forest+se), width=.1, position=pd) +
    geom_line(position=pd) +
    geom_point(position=pd, size=3, shape=21, fill="white")

p_plantation <- ggplot(summarySE(enrolled_df, measurevar="plantation", groupvars=c("first.treat","smallholder"), na.rm = TRUE), 
       aes(x=first.treat, y=plantation, colour=smallholder)) + 
    geom_errorbar(aes(ymin=plantation-se, ymax=plantation+se), width=.1, position=pd) +
    geom_line(position=pd) +
    geom_point(position=pd, size=3, shape=21, fill="white")

p_pasture <- ggplot(summarySE(enrolled_df, measurevar="pasture", groupvars=c("first.treat","smallholder"), na.rm = TRUE), 
...
p_ref <- ggplot(summarySE(enrolled_df, measurevar="reforestation2", groupvars=c("first.treat"), na.rm = TRUE), 
       aes(x=first.treat, y=reforestation2)) + 
    geom_errorbar(aes(ymin=reforestation2-se, ymax=reforestation2+se), width=.1, position=pd) +
    geom_line(position=pd) +
    geom_point(position=pd, size=3, shape=21, fill="white")

p_eco <- ggplot(summarySE(enrolled_df, measurevar="ecologio", groupvars=c("first.treat"), na.rm = TRUE), 
...
```{r landuse, fig.show="hold", out.width="32%"}
p_forest
p_plantation
p_pasture
```

## Reforestation and Ecologically valuable activities

I now show how take up of what conceivable are the projects with the highest carbon removal potential. I refer to these as "planting" and "ecologically valuable" projects. 

It is possible that these two trends correspond to the land use trends that were seen above. 

boxplots of the pasture and forest composition of these properties
...
We see that follow through and payment is relatively low. Amongst ther entire population of projects, only 44.4\% have been paid out. As mentioned in the data section, I am able to match approximately 85\% of payments to the associated property. Figure XX shows how payment varies across cohort years. While payouts seem relatively stable through 2017, recently enrolled properties have not yet completed projects and submitted the payment report. Figure XX also shows that smallholders are generally more likely to be paid than the group of larger properties. 

```{r paid, fig.show="hold", out.width="75%"}

p_payments

```

We also see that projects that fall under the 2nd and 4th year payout period are less likely to be paid out on. This is partly due to the fact that these properties will simply take longer to be paid. However, early cohort properties fall prey to this pattern as well, implying that these properties are less likely to ever be paid. This may imply somethhing about the profitability of these activities or the capability of these landowners to successfully complete the projects. 
...
```{r covar-bal, fig.cap = "Covarate balance between treated and untreated groups with and without matching. I follow ... and ... and consider the distributions to be matched if the normalized difference is less than 0.25.  "}
...
## Selection and a convincing counterfactual

...
compute_attgt_interaction <- function(dta, interact_var, interaction) {
  ...
  for (g in groups) {
    ...
    for (tp in time.periods) {
      ...
      this.data['outcome'] = this.data[interact_var]
      #this.data['outcome'] = this.data[, interact_var]
      
      mod <- glm(treat ~ forest + plantation + baresoil + pasture + urban +water+ shrub+ slope +lat+ elev  + property_area_ha  +road_dist +proportion_erosion+industry_dist +native_industry_dist  , data = this.data, family = "binomial")
      ...
      reg <- lm(Y ~  treat:post:outcome 
                + treat:post 
                + treat + post
                 + lat + forest + plantation + baresoil + pasture + urban +water+ slope + elev  + property_area_ha  +road_dist +proportion_erosion+industry_dist +native_industry_dist , data   = this.data
                , weights = psweights
      )
      
      if(interaction) {
        attgt <- reg$coefficients["treat:post:outcome"]
      } else{
        attgt <- reg$coefficients["treat:post"]
      }

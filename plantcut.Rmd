...
# 
# id_poly_y2 <- paste0(id_distinct$ID[1632:1700], collapse=",")
# write.csv(id_poly_y2, file = "id_poly_y2.txt", row.names = FALSE)
# st_write(id_distinct[1632:1700,], dsn = "id_poly_y2.json", driver = "GeoJSON")
...
``` {r did awarded, echo = FALSE, message = FALSE, warning = FALSE, include = TRUE, eval = TRUE}


did <- mp.spatt(NDVI ~ treat, data= awarded_panel,
                xformla=~ rptpro_monto_total + rptpro_puntaje + rptpre_superficie_bonificada + rptpre_superficie_predial + treated.later + as.factor(rptpre_region) ,
...
did <- mp.spatt(NDVI ~ treat, data= full_panel,
                xformla=~ rptpro_monto_total + rptpre_superficie_bonificada + rptpre_superficie_predial + as.factor(rptpre_region) ,
                 panel=TRUE, first.treat.name="first.treat", idname="ID", tname="Year",
...
# mp.spatt.test(NDVI ~ treat, data= full_panel,
#                  xformlalist=list(~rptpro_monto_total + rptpre_superficie_bonificada + rptpre_superficie_predial) ,
#                  panel=TRUE, first.treat.name="first.treat", idname="ID", tname="Year",
#                  method = "logit", # activate to use logit for propensity score matching
#                 , clustervarlist  = list("ID")
#                 )
...
did <- mp.spatt(NDVI ~ treat, data= smallholder_panel,
                xformla=~ rptpro_monto_total + rptpre_superficie_bonificada + rptpre_superficie_predial ,
                 panel=TRUE, first.treat.name="first.treat", idname="ID", tname="Year",
                 method = "logit", # activate to use logit for propensity score matching
                 bstrap=FALSE, se=TRUE, cband=FALSE

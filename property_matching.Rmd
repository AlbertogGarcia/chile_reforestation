---
title: "CIREN and Native Forest Law spatial Join"
...
## Overview statistics from spatial join
...
\begin{tabular}{|c|c|}
\hline
total\_projects & total Native Forest Law (NFL) projects\\
...
prop\_joined\_proj & proportion of NFL projects joined\\
rol\_match & proportion of stands with matching rol id\\ 
similar\_rol & proportion of stands with matching first part of rol id\\
stringd & avg. string distance$^*$ between property names from NFL and CIREN properties\\
...
$^*$stringd is calculated according to Damerau–Levenshtein method, which counts the number of operations (consisting of insertions, deletions or substitutions of a single character, or transposition of two adjacent characters) required to change one property name into the other


``` {r matching unprocesables, echo = FALSE, include = FALSE, eval = TRUE}

region_nombres <- df_stand %>%
  select(rptpro_region, rptpro_numero_region) %>%
  rename(Region = rptpro_region) %>%
...
# note that we can distinguish unprocesable vs unassigned projects by the presence of score

controls <- rejected %>%
  inner_join(prop_rural.sf, by = "ROL") #joining based on Rol id with CIREN data

#Need to account for regions 13-16
#this line of code ensures that Nuble and Los Rios obs have the correct region number
reg_match <- controls %>%
  mutate(region_CIREN = 
  ...
  filter(region_number == region_CIREN) #subset to only obs with same CIREN and NFL region number

##### Same with regions 3,4,11,12 ####
controls_12 <- rejected %>%
...
controls_11 <- rejected %>%
  inner_join(reg11.sf, by = "ROL") %>% #joining based on Rol id with CIREN data
  filter(Region == "Región de Aysen del General Carlos Ibáñez del Campo")

controls_3 <- rejected %>%
  inner_join(reg3.sf, by = "ROL") %>% #joining based on Rol id with CIREN data
  filter(Region == "Región de Atacama")

controls_extra <- rejected %>%
  inner_join(reg4.sf, by = "ROL") %>% #joining based on Rol id with CIREN data
  filter(Region == "Región de Coquimbo") %>%
  bind_rows(controls_3) %>%
  bind_rows(controls_12) %>%
  bind_rows(controls_11)

#####################################

#Joining to have all rejected projects
all_rejected <- reg_match %>%
  bind_rows(controls_extra) %>%
  mutate(DESCCOMU = 

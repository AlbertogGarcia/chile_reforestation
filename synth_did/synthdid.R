...
results_df <- readRDS("results_df.rds") %>%
  filter( first.treat <= 2015 )%>%#& rptpro_tipo_concurso == "Pequeños Propietarios" )%>%
  inner_join(test_ids, by = "id")

cutting_df <- results_df %>%
  filter(`corta-mejoramiento` == 1 | `corta-sanitaria` == 1 | `corta-selectiva` == 1 | `corta-liberacion` == 1 | `corta-recuperacion` == 1)

summary(lm(tau.2020 ~ rptpro_puntaje , data = results_df))

ggplot(results_df, aes(x = rptpro_puntaje, y = tau.2020, color = first.treat))+
  geom_point()+
  geom_smooth(method='lm', color = "red")+
  ylab("tau") + xlab("assigned score")+
  ylim(-0.1, 0.15)+
  ggtitle("treatment effects vs. assigned score")


summary(lm(tau.2020 ~ adjusted_puntaje , data = results_df))

ggplot(results_df, aes(x = adjusted_puntaje, y = tau.2020, color = first.treat))+
  geom_point()+
  geom_smooth(method='lm', color = "red")+
  ylab("tau") + xlab("adjusted score")+
 # ylim(-0.1, 0.1)+
  ggtitle("treatment effects vs. adjusted score")


summary(lm(tau.2020 ~ social_puntaje , data = results_df))

ggplot(results_df, aes(x = social_puntaje, y = tau.2020, color = first.treat))+
  geom_point()+
  geom_smooth(method='lm', color = "red")

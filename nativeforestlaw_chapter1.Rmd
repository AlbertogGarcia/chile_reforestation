...
Much research has addressed targeting in PES programs in the context of avoided deforestation. The key to gaining high environmental effectiveness at least cost is to enroll only the parcels at risk of deforestation [@alix-garcia_only_2015]. This can be achieved by providing adequate payments and focusing on parcels in areas of high deforestation. This is not the case in the reforestation context, where high environmental effectiveness is determined by whether landowners can generate and maintain new forest cover. The characteristics likely to determine effectiveness in the reforestation setting may thus be less easily targeted than in the deforestation setting. We briefly outline the program's scoring mechanism here, but more detailed explanation can be found in the appendix. 

Applications in both the smallholder and other interested party contest are scored based on a predetermined set of criteria, that include factors related both to landowner, property, and project characteristics. This mechanism determinses funding priority. Funding is allocated first to the smallholder contest with the highest scoring smallholders awarded first. After funding has been allocated to qualifying smallholders, the remaining funds are given to other interested parties with the highest scores until funding runs out. In some yyears, a second contest was held only for smallholders, and these are the only contests in which smallholders were not granted funding due to the scoring mechanism. This score, although not always critical for smallholder applicants, provides insight to program administrators' preferences for project prioritization. 
...
The parameters $\beta_1$ ; $\beta_2$; $\beta_3$; $\beta_4$ vary slightly across contest. For reference, in the 2018 contest, $\beta_1 = 0.$ , $\beta_2 = 0.$; $\beta_3 =0.$, and $\beta_4 =0.$. 

Other interested parties are scored according to:

$$ score_{other} =   $$ 

For reference, the 2018 contest used $\gamma_1 = 0.$ , $\gamma_2 = 0.$, and $\\gamma_3 =0.$. 

Figure XX plots the treatment effects, $ATT_{ovr}$ for each score quartile. Note that score quartiles are determined by cohort, so each quartile contains equal cohort representation. In order for the score to be a good predictor of additionality, a primary goal of the program, we should see that treatment effect estimates increase across each quartile of the score. We see that 
...
Targeting based on social factors such as indigenous status or property size may come with costs or benefits in terms of additionality. In order to understand how the social component influences the score's association with our treatment effect estimates, we recalculate project scores, excluding the influence of $VPS$ and $VI$. Now, each score only consits of elements from $VT$ and $VP$. Figure XX shows that the association of our adjusted score with the treatment effect estimates is not clearly different than that of the true score. 
...
Explanation: even aspects of the project score are determined by political economy factors such as blah blah. In the next section, we explore how observables drive treatment effect heterogeneity before showing that accounting for these factors when scoring projects could result in significantly better prediction of additionality. 
...
In section XX, we showed that the program's scoring criteria had a positive but slight association with treatment effect estimates. Based on the drivers of treatment effect heterogeneity, we construct an alternative scoring mechanism. We show that targeting based on these observables outperforms both the true and adjusted scores assigned by the program. 

We begin with a simple score that multiplies the estimated coefficient from each covariate by each property's value of the covariate. As such, the score is created as follows:

$$ score_{observable} = \sum^n_{k=1} \beta_{k, ovr} Z_i$$

, where ...

Figure XX plots the treatment effect estimates against $score_{observable}$. We see that ...

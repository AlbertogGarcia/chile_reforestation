...
reg4.sf <- st_as_sf(reg3.shp) %>%
  st_set_crs("+proj=utm +zone=19 +south +ellps=aust_SA +units=m +no_defs")
reg11.sf <- st_as_sf(reg3.shp) %>%
  st_set_crs("+proj=utm +zone=19 +south +ellps=aust_SA +units=m +no_defs")
reg12.sf <- st_as_sf(reg3.shp) %>%
  st_set_crs("+proj=utm +zone=19 +south +ellps=aust_SA +units=m +no_defs")

properties.sf <- prop_rural.sf %>% 
  st_join(reg3.sf, join = st_union) %>%
...
join_assess_df <- df_joined %>%
  select(rptpro_id, ROL, rptpre_rol, NOM_PREDIO, rptpre_nombre) %>%
...
### if property name is the same
### if property name is similar (shares word longer than 3 letters)

...
id_poly_y <- paste0(id_distinct$ID[1626:1700], collapse=",")
write.csv(id_poly_y, file = "id_poly_y.txt", row.names = FALSE)
st_write(id_distinct[1626:1700,], dsn = "id_poly_y.json", driver = "GeoJSON")
...
rejected_panel <- rejected_ndvi %>%
  inner_join(NLA_df, by = "polygon_id") %>%
  mutate(first.treat = 0,
         treat = 0
         ) %>%
  filter(Year < 2019) %>%

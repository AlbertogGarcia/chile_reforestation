---
title: "Targeting and compliance in Chilean payments for native forest recovery"
...
Much research has addressed targeting in PES programs in the context of avoided deforestation. The key to gaining high environmental effectiveness at least cost is to enroll only the parcels at risk of deforestation [@alix-garcia_payment_2014]. This can be achieved by providing adequate payments and focusing on parcels in areas of high deforestation. In these contexts, however, it has been difficult to achieve the dual objectives of environmental efficacy and participation of low-income, rural, and indigenous communities. This is driven by the fact that these communities often have relatively lower deforestation risk [e.g., @alix-garcia_only_2015; @pfaff2007].

This is not the case in the reforestation context, where high environmental effectiveness is determined by whether the payment induces landowners to generate and maintain new forest cover through sustained behavior change. As such, non-compliance may play a major role if, for example, landowners are unertain as to the costs or benefits of generating forest cover [e.g., @oliva_technology_2020]. For simplicity, we consider the probability of noncompliance to be an underlying feature of the setting and do not consider its potential causes. In order to illustrate how conservation costs and noncompliance determine the effectiveness of program administrators' targeting strategy, we adapt a graphical framework developed in @jack_self_2019 to address the effectiveness of programs' targeting strategies in these contexts. 

Suppose that landowner $i$ is oe on 15 landowners and experiences cost $c_i$ (ranging from -5 to 10) to produce forest carbon benefits equal to 10. A PES program is introduced that pays landowners 5 in order to provide that forest carbon. This framework is applicable to either the avoided deforestation or reforestation setting, with forest carbon provided either through conservation or by adding additional forest cover. 

Figure XX depicts the aggregate supply of carbon at a given conservation cost in this setting. For some of these landowners, there is a negative cost to producing forest-based carbon, meaning that they provide it without the existence of any PES program. These are not cost-effective enrollees, as they produce no additional carbon relative to the no-PES scenario. Landowners with costs to producing carbon between 5 and 10 apply to the program and provide carbon that would not have been provided in the absence of the program. Landowners with costs over 5 do not apply to the program, as they would still experience negative costs of production after the PES payment. For simplicity, we assume that the landowner with 
...
Figure XX shows the case where $Z_i$ is positively correlated with costs to producing forest carbon. In this case, program administrators enroll all applicants (Right panel of Figure XX) that provide additional forest carbon and only inframarginal applicants are not admitted. This is representative of the case where program administrators truly experience win-wins, and the targeting strategy achieves a cost-effective program design and enrolls landowners with high values of the priority targeting characteristic, $Z_i$. 
...
Assume that payment is conditional on successful provision of forest carbon. If non-compliance probability is uncorrelated with $Z_i$, overall environmental impact of the program is decreased relative to the full-compliance case, however, the cost-effectiveness of the successful payments is not affected. Table XX shows how positive and negative correlation between non-compliance and $Z_i$ influences the cost-effectiveness of the program when using $Z_i$ as a targeting characteristic. 

In order to provide intuition, we discuss the case when $Z_i$ is negatively correlated with $c_i$, and non-compliance probability is positively correlated with $Z_i$. In this case, those with high values of $Z_i$ become relatively less likely to follow-through and receive payment. These also happen to be the inframarginal applicants, so overall cost-effectiveness is improved relative to the full-compliance case. 
...
This framework generates key conditions that are relevant for our exploration of the targeting strategy used in the native forest law. 

Win-wins of environmental effectiveness and prioritization of priority groups are more likely under the following circumstances:

\begin{enumerate}
\item when prioritized applicants are relatively more likely to have positive costs of producing forest carbon (i.e., they were unlikely to have provided carbon benefits without the existence of the program)
\item when non-compliance is uncorrelated or negatively correlated with prioritized applicant criteria
\end{enumerate}

In the deforestation case, costs of producing forest carbon are determined by deforestation risk. It is easy to see from this framework that achieving win-wins is unlikely if prioritized groups have lower deforestation risk and non-compliance is not an issue. We explore these factors in the reforestation case in the context of Chile's Native Forest Law. 

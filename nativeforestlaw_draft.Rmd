...
# When is targeting for co-benefits efficient

## A conceptual framework for reforestation

A model or more formal framework could probably help this section. 
...
In our case, program administrators target on property and landowner characteristics that are likely correlated with program co-benefits such as poverty alleviation and community participation. In order to achieve this, the program is willing to pay certain groups higher prices and to ensure certain groups are more likely to receive funding. Appendix XX (need to add still) shows the pricing schemes for both the smallholder and other interested party contests. Note that activities in the smallholder contest are paid at a 15\% higher rate than those in the other interested party contest. Appendix XX (still need to add) shows the scoring scheme for each contest. 

## Approach

To understand better understand whether Native Forest Law targeting was efficient, we generate property specific treatment effects and explore their relationship with the program's targeting mechanisms. Our matched control group along with synthetic difference-in-differences allow us to estimate property specific treatment effects. Program documents detailing the payment and scoring rules allow us to recreate project scores and bonus amounts, before adjusting them to exclude the score and payment bonuses given to smallholders and indigenous peoples. We then explore treatment effect heterogeneity and explore how this heterogeneity corresponds to projects scores and lanowner payments, both before and after adjusting for program targeting goals. 

<!-- ### Synthetic DID -->

<!-- Need to outline methods still. Figure \@ref(fig:sdid_example) shows synthetic difference-in-differences results for one treated property. The left panel shows the trajectory of the synthetic control property's EVI alongside that of the treated property. The right panel overlays these trajectories in order to get a sense of the true treatment effect and examine pre-trends.  -->

<!-- ```{r sdid-example, fig.show="hold", out.width = '49%', fig.cap = "This figure shows the results of our synthetic difference-in-differences procedure for one treated property.", echo=FALSE} -->
<!-- knitr::include_graphics(c("figs/sdid.png", "figs/sdid_overlay.png")) -->

<!-- ``` -->

### Adjusting scores and payments for targeting goals

### Results and implications for targeting

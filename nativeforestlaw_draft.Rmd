---
title: "Targeting and compliance in Chilean payments for native reforestation"
...
\item integrate woody biomass/land-cover classification maps and estimates
\end{itemize}
...
# Targeting for multiple objectives with heterogeneous compliance: a framework

Much research has addressed targeting in PES programs in the context of avoided deforestation. The key to gaining high environmental effectiveness at least cost is to enroll only the parcels at risk of deforestation [@alix-garcia_payment_2014]. This can be achieved by providing adequate payments and focusing on parcels in areas of high deforestation. This is not the case in the reforestation context, where high environmental effectiveness is determined by whether the payment induces landowners to generate and maintain new forest cover through sustained behavior change. As such, non-compliance may play a major role if, for example, landowners are unertain as to the costs or benefits of generating forest cover [e.g., @oliva_technology_2020]. For simplicity, we consider the probability of noncompliance to be an underlying feature of the setting and do not consider its potential causes. 
In order to understand how noncompliance impacts program administrators' targeting strategy, we discuss a simple rent-driven model of land use [e.g., @pfaff1999] and take a graphical approach similar to @alix-garcia_only_2015. Let there be a set of landowners who vary in some characteristic, $Z_i$, that is used by program administrators to target payments. Let $r_f$ describe the rents to forest land use and $r_a$ describe the highest rent of any alternatives. Figure \@ref(fig:framework) describes the rents to forest and alternative land uses as a function of $Z_i$. 

Prior to the existence of the program, landowners with values of $Z_i$ less that $x_1$ would allocate their land to non-forest uses. The introduction of the program shifts $r_f$, and anyone with a value of $Z_i$ greater than $x_0$ (the point where $r_a = r_f + PES$) should apply to the program. 

From the perspective of program administrators, landowners at $x_0$ are the most cost-effective enrollees, since the offered payment is equal exactly to the minimum payment required to shift them to forest land uses. Further, payments to landowners with a value of $Z_i$ greater than $x_1$ are likely to be inframarginal. This means that amongst those who apply to the program (i.e. those with $Z_i \geq x_0$), administrators should target payments to those with low values of $Z_i$ first. 

```{r framework, fig.show="hold", fig.pos="H", out.width = '100%', fig.cap = "Graphical rent model, where the x-axis shows a characteristic used for targeting that also affects rents"}

knitr::include_graphics("figs/framework.png")
```

We can now think about how noncompliance may influence the environmental impact and cost-effectiveness of payments in this context. Also assume that payments are not made unless landowners do comply. If compliance probability is uncorrelated with $Z_i$, overall environmental impact of the program is decreased, however, the cost-effectiveness of the successful payments is not hurt.  

In the case where compliance probability is positively correlated with $Z_i$ (i.e. those with high values of $Z_i$ are more likely to comply), not only will the program lose environmental impact, but cost-effectiveness of the payments will decline. This is because the most cost-effective applicants (i.e. those with the lowest values of $Z_i$) will be least likely to comply, and those who are inframarginal are more likely to receive payment. As such, the negative correlation between program impact and compliance creates tension for program administrators. Lastly, if compliance probability is negatively correlated with $Z_i$, cost-effectiveness will improve, and impact will improve relative to the previous two cases (assuming the overall compliance rate is the same).

\textbf{Note:} One source of tension in this framework is how noncompliance arises. I really want to remain agnostic as to the source of noncompliance, however, it may help to ground ideas. My main obstacle is thinking about the case in which noncompliance is positively correlated with $Z_i$. In this case, those who would already allocate land to forest uses are less likely to comply. This is hard to make sense of intuitively, since they have the least to do in order to receive the payment.


# Evaluating the Native Forest Law's targeting strategy

In the case of the Native Forest Law, targeting is done in two main ways: 1) holding separate contests for smallholders and larger properties, where the smallholder contest pays 15\% higher; and 2) a scoring mechanism that prioritizes indigenous peoples, smaller properties, and other landowner and property characteristics. Here we separately evaluate the scoring system for the smallholder and other interested party contests. 
...
Targeting based on social factors such as indigenous status or property size may come with costs or benefits in terms of both environmental impacts and compliance. In order to focus on the influence of the program's targeting of social characteristics, I isolate the influence of social characteristics on the score. As such, I consider an alternative score to examine. This adjusted score considers the project score excluding any social components, by subtracting the social components from the true assigned score in order to focus solely on land and activity specific components. 

## Results

### smallholder contest

Compliance plays a major role in successful targeting for the Native Forest Law, particularly because it is so low. We now explore the relationship between the assigned project score and compliance in the smallholder contest. Figure \@ref(fig:compliance-small) shows that the assigned project scores were positively correlated with compliance in the smallholder contest.

```{r compliance-small, fig.show="hold", fig.pos="H", out.width = '60%', fig.cap = "The assigned score is possitively associated with compliance in the smallholder contest", echo=FALSE}
knitr::include_graphics("figs/small_compliance_plot.png")
```

Figure \@ref(fig:qdid-small) plots $ATT_{ovr}$ for each score quartile of the assigned score. Note that we determine score quartiles by cohort, so each quartile contains equal cohort representation. In order for the score to be a good predictor of increased forest cover, a primary goal of the program, we should see that treatment effect estimates increase across each quartile of the score. Figure \@ref(fig:qdid-small) shows a slight positive correlation between the assigned project scores and treatment effects. Perhaps most importantly, those with the lowest scores had the least impact, meaning that applicants rejected because of low scores where unlikely to provide much benefit anyway. These results suggest that the program's scoring system successfully targeted projects that were likely to be impactful. Further, because compliance was postitively correlated with the scores, and thus with treament effects, the program's targeting strategy improved cost-effectiveness within the smallholder contest. 
...
### other interested parties contest

In the other interested party contest, we see that the score was neither correlated with compliance or treatment effects. In fact, no score quartile produced treatment effect estimates that were statistically different from zero. This may reflect the fact that enrollees in the other interested parties contest are larger landowners more likely to maintain forest even in the absence of the program. We see in Table \@ref(tab:) that other interested parties on average have more plantation forest than smallholders, possibly indicating that they would have generated some level of forest cover even in the absence of the program. As such, the program's strategy to prioritize smallholder may be beneficial from a targeting perspective. 


```{r qdid-other, fig.show="hold", fig.pos="H", out.width = '49%', fig.cap = "Assigned score in the other interested parties contest is not clearly associated with Left: compliance; and Right: treatment effects", echo=FALSE}
knitr::include_graphics(c("figs/other_compliance_plot.png", "figs/other_q_assigned.png"))
```

## influence of social targeting

Targeting based on social factors such as indigenous status or property size may come with costs or benefits in terms of additionality. In order to understand how the social component influences the score's association with our treatment effect estimates, we recalculate project scores, excluding the influence of $VPS$ and $VI$. Now, each score only consists of elements from $VT$ and $VP$. The left panel of Figure \@ref(fig:small-adj) shows that the association of our adjusted score with compliance is slightly different than that of the true score. The difference in slopes is statistically different at the 10\% level. This indicates that targeting on social characteristics may have come at a small cost in terms of compliance. The right panel shows that the association of our adjusted score with $ATT_{ovr}$ is not clearly different than that of the true score. However, the $ATT_{ovr}$s are monotonically increasing in adjusted score quartile, while they were not in the assigned scores. This may be an indication of a more informative score. Ultimately, it appears that any costs of targeting based on social characteristics were rather small. 


```{r small-adj, fig.show="hold", fig.pos="H", out.width = '49%', fig.cap = "In the smallholder contest, the adjusted score is Left: a better predictor of compliance than the assigned score; and Right: not clearly a better predictor of impact", echo=FALSE}
knitr::include_graphics(c("figs/compliance_smallholder2.png", "figs/small_q.png"))
```

Figure \@ref(fig:other-adj) shows how targeting on social characteristics may have influenced the score's value in the other interested party contest. There is no clear difference in the association of the adjusted score with compliance or impact relative to the assigned score.  

```{r other-adj, fig.show="hold", fig.pos="H", out.width = '49%', fig.cap = "In the other interested party contest, the adjusted score is no better or worse at predicting compliance or impact than the assigned score", echo=FALSE}
knitr::include_graphics(c("figs/compliance_other2.png", "figs/other_q.png"))
```
...
Table \@ref(tab:compliance-covars) shows that landowners who follow-through differ from those who do not on a number of characteristics. This includes project and property characteristics. Notably, projects that do follow-through are likely to belong to properties with more plantation forest, but there is no detectable difference in native forest. One potential driver of compliance is access to extensionist agents. The probability of having an extensionist is drastically different between the two groups. Access to extensionists may help explain why smallholders have a higher compliance rate than other interested parties. Future iterations of this paper will take a more active approach to predicting compliance and understanding the potential foregone benefits from a lack of compliance. 
...
\textbf{Note:} Not updated, so feel free to ignore

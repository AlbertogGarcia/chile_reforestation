...
activity_df <- property_df %>%
  ...
  mutate(reforestation = ifelse(plantacion == 1 | `plantacion-suplementaria`==1 | regeneracion==1 , 1, 0),
         ...
         ref_indicyr = ifelse(reforestation ==1 | rptpro_objetivo_manejo == "BOSQUE PRESERVACION Y FORMACIONES XEROFITICAS DE ALTO VALOR ECOLOGICO", rptpro_ano, NA),
         ref2_indicyr = ifelse(reforestation2 ==1 | rptpro_objetivo_manejo == "BOSQUE PRESERVACION Y FORMACIONES XEROFITICAS DE ALTO VALOR ECOLOGICO", rptpro_ano, NA),
...
property_df <- property_df %>%
  mutate(rptpre_comuna = tolower(accents(rptpre_comuna)))%>%
  select(rptpre_comuna, rptpre_rol)

library(rio)
export(property_df, "property_df.rds")

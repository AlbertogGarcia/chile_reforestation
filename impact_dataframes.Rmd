...
activity_df <- property_df %>%
  left_join(rodal_df, by = "rptpre_id") %>%
  left_join(actividades_df, by = "rptro_id") %>%
  dcast(prop_ID +rptpre_rol + rptpro_objetivo_manejo + rptpre_comuna + rptpro_ano
        ~ rptac_tipo, 
...
missed_submitted <- submitted_mp %>%
  anti_join(submitted_rol, by = c("prop_ID")) %>%
  mutate(desccomu = rptpre_comuna)

missed_regions <- missed_submitted %>%
  select(rptpre_region, rptpro_numero_region)%>%
  distinct()

examine_missed_submitted <- prop_rural %>%
  left_join(missed_regions, by = c(region = "rptpro_numero_region"))%>%
  right_join(missed_submitted, by = c("desccomu", "rptpre_region"))%>%
  separate(rptpre_rol.y, into = c("rol_left.y", "rol_right.y"), sep = "-")%>%
  separate(rptpre_rol.x, into = c("rol_left.x", "rol_right.x"), sep = "-")%>%
  group_by(prop_ID)%>%
  mutate(rol_distance = stringdist(rptpre_rol.y, rptpre_rol.x),
         area_diff = abs(area_ha-rptpre_superficie_predial),
         area_prop = area_diff/rptpre_superficie_predial,
         rol_left.y=str_remove(rol_left.y, "^0+") ,
         rol_left.x=str_remove(rol_left.x, "^0+") ,
         rol_right.y=str_remove(rol_left.y, "^0+") ,
         rol_right.x=str_remove(rol_left.x, "^0+") ,
         rol_left = stringdist(rol_left.y, rol_left.x),
         rol_right = stringdist(rol_right.y, rol_right.x)
         )%>%
  filter((rol_distance <=1&area_prop <=1/4)|(rol_left==0&rol_right==0))%>%
  select(prop_ID, rol_distance, area_ha, rptpre_superficie_predial, rol_left, rol_right, rptpre_rol.y, rptpre_rol.x, rptpre_nombre, NOM_PREDIO, rptprop_nombre, PROPIETARI)

submitted_geoms <- submitted_rol %>%
  select(prop_ID)

submitted_geoms1 <- submitted_rol %>%
  filter(rptpre_region == "Región del Maule"|
         rptpre_region == "Región de Ñuble"|
         rptpre_region == "Región del Bío-Bío"|
         rptpre_region == "Región de Los Lagos"|
         rptpre_region == "Región de Los Ríos"|
         rptpre_region == "Región de La Araucanía"
         )%>%
  select(prop_ID)

submitted_geoms2 <- submitted_rol %>%
  filter(rptpre_region != "Región del Maule"&
         rptpre_region != "Región de Ñuble"&
         rptpre_region != "Región del Bío-Bío"&
         rptpre_region != "Región de Los Lagos"&
...
table(submitted_rol$rptpre_region)
st_write(submitted_geoms1, dsn = "submitted_geoms1.shp", driver = "ESRI Shapefile")
st_write(submitted_geoms2, dsn = "submitted_geoms2.shp", driver = "ESRI Shapefile")

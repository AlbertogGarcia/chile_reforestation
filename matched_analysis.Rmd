...
did.mgmt.es <- aggte(did.mgmt, type="dynamic", na.rm = TRUE, min_e=-13)#, max_e = 10)
ggdid(did.mgmt.es)
did.mgmt.ovr <- aggte(did.mgmt, na.rm=TRUE, type = "simple")

dyn.es <- data.frame("att" = did.mgmt.es$att.egt, "se" = did.mgmt.es$se.egt, "e" = did.mgmt.es$egt, "crit.val" = did.mgmt.es$crit.val.egt)%>%
...
source('lu_did_panel.R')
source('lu_compute_attgt.R')
source('lu_att_gt.R')
set.seed(0930)
...
did.landuse <- lu_att_gt(yname="EVI2",
                     ...
                     data=mgmtplan_analysis, clustervars = "id"
                     , panel=TRUE, bstrap = TRUE
                     , lu_varname = "forest", interaction=TRUE
                     )

did.lu.es <- aggte(did.landuse, type="dynamic")

```

``` {r colinear with treatment interactions, echo = FALSE, message = FALSE, warning = FALSE, include = TRUE, eval = TRUE}

set.seed(0930)

mgmtplan_analysis <- mgmtplan_analysis %>%
  mutate(reforestation2 = ifelse(is.na(reforestation2), 0, reforestation2),
         smally = ifelse(smallholder==1&treat==1, 1, 0),
         rptpro_puntaje = ifelse(is.na(rptpro_puntaje), 0, rptpro_puntaje),
         female = ifelse(rptprop_sexo=="Masculino" | is.na(rptprop_sexo), 0, 1),
         indigenous = ifelse(is.na(rptprop_etnia), 0, 1),
         extensionist = ifelse(rptpro_tipo_presenta=="Extensionista", 1, 0)
         )
...
did.mgmt <- att_gt(yname="EVI2",
                     ...
                     idname="id",
                     gname="first.treat",
                     est_method = my_did_panel2,
                     xformla=~  indigenous + forest + plantation + baresoil + pasture +shrub + urban +water+ slope +lat+  elev  + area_ha  +road_dist ,
                     data=mgmtplan_analysis, clustervars = "id"
                     , panel=TRUE, bstrap = TRUE
                     )

did.mgmt.es <- aggte(did.mgmt, type="dynamic")
...
```{r}
library(synthdid)
sdid_data <- mgmtplan_analysis %>%
  mutate(treatment = as.numeric(ifelse(first.treat>0 & Year >= first.treat, 1, 0)))%>%
  filter(first.treat==2009 | first.treat==0)%>%
  select(id, Year, NDVI, treatment)

length(unique(sdid_data[, sdid_data$treatment]))

setup = panel.matrices(sdid_data)
tau.hat = synthdid_estimate(setup$NDVI, setup$id, setup$treatment)
...
# treatpostplant.results <- res$dyn.ovr
# treatpostplant.results$att.se <- boot.se
# treatpostsmall.results <- res$dyn.ovr
# treatpostsmall.results$att.se <- boot.se

treatpostsmallplant.results <- res$dyn.ovr
treatpostsmallplant.results$att.se <- boot.se

# treatpost <- compute.attgt(dta)
# treatpostplant <- compute.attgt(dta)
#treatpostsmall <- compute.attgt(dta)
#treatpostsmallplant <- compute.attgt(dta)

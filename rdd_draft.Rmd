...
$$ X_i = \mathbb{1} \gamma_1\{ R_i = Z \} + \gamma_2(R_i - Z) + \epsilon_i$$

, where $Z$ is the heap point of interest, and $R_i$ is the reported property size for project $i$. 

Figure XX reports $\gamma_1$ at various heap points across several application characteristics. 
...
knitr::include_graphics(c("figs/heap_covars_extensionista.png", "figs/heap_covars_received_bonus.png", "figs/heap_covars_digital.png"))
```
...
Lastly, we explore the properties who have reported property sizes just under the threshold. Figure \@ref(fig:heapers-200) shows the reported and true property sizes for these properties after limiting the sample to only matched properties with a reported property size between 198 to 200 hectares. We see that not all of the properties are underreporting their property size. Some are likely unsure of their exact property size and simply round to the nearest 10 hectares or so. Roughly half of the properties bunching just below the threshold had true property sizes below the threshold, although very few had accurately reported sizes. 

```{r heapers-200, fig.pos="H", out.width = '90%', fig.cap = "test", echo=FALSE}
knitr::include_graphics("figs/heapers_200.png")
```

## Are heapers different?

I first explore how the smallholder heapers at 200 ha differ from other interested party enrollees just above the 200 ha threshold. Table 1 shows that the heapers are statistically different from other interested parties in almost no way. The exception is that the smallholders are paid at much higher rates. This may be due to the higher payment or signal that the heapers are somehow different from the other properties in some way.
...
kable(results_df, format = "latex", row.names = FALSE, booktabs = T,
      ...
      col.names = c("variable", "heaper mean", "smallholders (180 to 198 ha) mean", "p-value"),
      escape = TRUE) %>%
  kable_classic()
...
test_df <- discontinuity_main %>%
  filter(between(property_size, 1, 1000))
# Welch t-test
timber <- t.test(timber ~ round_heaper, test_df)

digital <- t.test(digital ~ round_heaper, test_df)

ecological_recovery <- t.test(ecological_recovery ~ round_heaper, test_df)

extentionist_consultant <- t.test(extentionist_consultant ~ round_heaper, test_df)

bonus_area <- t.test(rptpro_superficie ~ round_heaper, test_df)

psize <- t.test(property_size ~ round_heaper, test_df)

rptpro_monto_total <- t.test(rptpro_monto_total ~ round_heaper, test_df)

rptpro_puntaje <- t.test(rptpro_puntaje ~ round_heaper, test_df)

received_bonus <- t.test(received_bonus ~ round_heaper, test_df)

results_df <- data.frame("variable" = c("timber production", "ecological recovery", "digital application", "used extensionist or consultant", "bonus area", "property size", "payment (UTM)", "project score", "received payment"), 
  "heaper_mean" = c(timber$estimate[2], ecological_recovery$estimate[2], digital$estimate[2], extentionist_consultant$estimate[2], bonus_area$estimate[2], psize$estimate[2], rptpro_monto_total$estimate[2], rptpro_puntaje$estimate[2], received_bonus$estimate[2]),
  "other_mean" = c(timber$estimate[1], ecological_recovery$estimate[1], digital$estimate[1], extentionist_consultant$estimate[1], bonus_area$estimate[1], psize$estimate[1], rptpro_monto_total$estimate[1], rptpro_puntaje$estimate[1], received_bonus$estimate[1]),
  "p value" = c(timber$p.value, ecological_recovery$p.value, digital$p.value, extentionist_consultant$p.value, bonus_area$p.value, psize$p.value, rptpro_monto_total$p.value, rptpro_puntaje$p.value, received_bonus$p.value)
)

kable(results_df, format = "latex", row.names = FALSE, booktabs = T,
      caption = "differences in means for multiple of 10 heapers vs. non-heapers",
      col.names = c("variable", "round number heaper mean", "other mean", "p-value"),
      escape = TRUE) %>%
  kable_classic()
...
- is this a cost placed on landowners who cannot afford to hire surveyor to learn true property size?
- is this costly to program administrators/ do heapers take away benefits from smallholders?
- what are the benefits of heaping?
